/**
 * Pure grading settlement helpers (2C-2J-6A).
 *
 * Moneyline realized PnL uses sportsbook American odds (closePrice), NEVER modelPrice.
 * modelPrice remains fair-value input for CLV only.
 *
 * Keep in sync with apps/jobs/lib/grading-settlement.ts (jobs build boundary).
 */
...
/** Sportsbook American odds usable for payout / CLV. */
export function isValidAmericanOdds(price: number): boolean {
  return Number.isFinite(price) && price !== 0;
}

/**
 * Winning American-odds PnL from the sportsbook/wager price.
 * Fail closed on invalid odds (do not substitute model fair odds).
 */
...
/**
 * 2026 official_flat_100 cards persist wager closePrice at write time.
 * Missing closePrice is an invariant failure — do not reconstruct from MarketLine.
 */
...
export function gradeSpreadTotal(
  ...
  // Flat -110 assumption for spreads/totals (unchanged).
...
/**
 * Grade moneyline. Realized PnL uses sportsbook closePrice only.
 * CLV retains implied(modelPrice) - implied(closePrice).
 */
...
/**
 * Side-aware pre-kick closing line. Never uses post-kick observations.
 * spread/moneyline require selected teamId; total does not.
 */
