 ...
 * 
 * Simulates what happens to the official_flat_100 card if we drop all
...
async function main() {
  ...
  // Fetch all graded official_flat_100 bets for the season
  ...
  console.log(`\nFound ${betsRaw.length} graded official_flat_100 bets`);
  ...
  for (const bet of betsRaw) {
    ...
    if (betTeamContinuity === null && bet.marketType === 'spread') {
  ...
  // Print results
  formatStats(baselineStats, 'Baseline Official Card (all bets)');
  formatStats(removedStats, 'Removed subset – Low-Continuity Dogs (spreads only)');
  formatStats(filteredStats, 'Filtered Official Card (dropping low-continuity dogs)');

  // Calculate delta
  const betsRemoved = removedBets.length;
  const pnLImprovement = filteredStats.totalPnL - baselineStats.totalPnL;
  const roiChange = filteredStats.roi - baselineStats.roi;

  console.log(`\nDelta vs Baseline:`);
  console.log(`    Bets removed: ${betsRemoved}`);
