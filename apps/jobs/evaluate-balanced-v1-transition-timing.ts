#!/usr/bin/env node
/**
 * Phase 2C-2H-7 — Read-only Balanced V1 transition timing evaluation.
 *
 * Usage:
 *   npx tsx apps/jobs/evaluate-balanced-v1-transition-timing.ts --study-season 2025
 *
 * SELECT-only. No CFBD. No Odds. No ratings writes.
 * Does not authorize transition/bridge/persistence.
 */
...
import {
  ...
  formatBalancedV1TransitionTimingReport,
...
export interface BalancedV1TransitionTimingReadStore {
...
export function createPrismaBalancedV1TransitionTimingReadStore(
  prisma: PrismaClient
): BalancedV1TransitionTimingReadStore {
  return {
...
export async function runBalancedV1TransitionTimingEvaluation(options: {
  studySeason: number;
  store: BalancedV1TransitionTimingReadStore;
}): Promise<{ exitCode: number }> {
  if (options.studySeason !== STUDY_SEASON_2025) {
    console.error(
      `[balanced-transition] studySeason must be ${STUDY_SEASON_2025}`
    );
    return { exitCode: 1 };
  }
  ...
  try {
    ...
    console.log(formatBalancedV1TransitionTimingReport(result));
    if (!result.ok) {
      console.error(
        '[balanced-transition] FAILED — structural gates not met'
      );
      return { exitCode: 1 };
    }
    console.log(
    ...
    console.error(
      `[balanced-transition] ${sanitizeBalancedV1TransitionTimingError(err)}`
    );
    return { exitCode: 1 };
  }
}
...
async function main(): Promise<void> {
  const parsed = parseBalancedV1TransitionTimingArgs(process.argv.slice(2));
  if (parsed.ok === false) {
    for (const e of parsed.errors) {
      console.error(`[balanced-transition] ${e}`);
    }
    console.error(
      'Usage: npx tsx apps/jobs/evaluate-balanced-v1-transition-timing.ts --study-season 2025'
    );
    process.exit(1);
  }
  ...
  try {
    const { exitCode } = await runBalancedV1TransitionTimingEvaluation({
      studySeason: parsed.studySeason,
      store: createPrismaBalancedV1TransitionTimingReadStore(prisma),
    });
